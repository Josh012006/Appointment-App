    ...
    return(
        <>
            ...
            <div className="flex flex-col">
                ...
                {(!clicked) && isFocused && (OptionsTab.length !== 0) && <div className="bg-white relative border-2 border-black rounded-lg" style={{maxHeight: '300px', overflowY: 'auto'}}>{OptionsTab.map((option) => {return <option onClick={() => {setChoice(option); setClicked(true);}} className="m-0 border-b min-h-12 border-black p-3 relative cursor-pointer whitespace-normal" key={option}>{option}</option>})}</div>}
            </div>
        </>
    );
}
